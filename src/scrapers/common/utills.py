...
def clean_text(text: str | None):
    """Xóa khoảng trắng thừa (xuống dòng, tab, nhiều dấu cách liên tiếp)."""
    if not text:
        return None
...
def get_text(soup: BeautifulSoup, selector: str):
    """Lấy text (đã làm sạch) của phần tử đầu tiên khớp với selector CSS."""
...
def get_project_root(current_file: str, levels_up: int = 3):
    """
    Trả về thư mục gốc của project, tính từ file gọi hàm (current_file),
    lùi lên `levels_up` cấp thư mục.
    """
...
def get_output_csv_path(project_root: Path, site_name: str, filename: str = "listing.csv"):
    """
    Trả về đường dẫn file CSV output cho 1 trang web cụ thể,
    theo cấu trúc: data/raw/<site_name>/<filename>
    """
...
def save_to_csv(
    ...
    """
    Ghi thêm danh sách (tiêu đề, link) vào file CSV, có STT tự tăng.
    Tự tạo thư mục cha nếu chưa tồn tại. Tự ghi header nếu file chưa tồn tại.
    """
...
def load_column_from_csv(path: Path, column: str, limit: int | None = None):
    """Đọc 1 cột từ file CSV, trả về danh sách giá trị (bỏ dòng trống)."""
    values = []
...
def save_dict_rows_to_csv(
    ...
    """Ghi danh sách dict vào file CSV."""
    path.parent.mkdir(parents=True, exist_ok=True)
...
def get_next_page_url(
    ...
    """
    Tìm URL của trang kế tiếp trong khối phân trang.
    Selector và ký hiệu nút "next" có thể tùy chỉnh cho từng trang web
    vì mỗi trang có cấu trúc phân trang hơi khác nhau.
    """
